 ...
 */
test.describe("Login to ShopBase and select one shop to go <1> @TS_SB_DASHBOARD_01 @PERF", async () => {
  test("Login and verify <1> @TC_SB_DB_LGIN_01 @PERF_01", async ({ page, conf }) => {
    ...
    await test.step("Verify if we are landing to the right store", async () => {
      expect(await page.locator(`(//p[contains(@class, 'truncate')])[2]`).textContent()).toBe(conf.caseConf.domain);
    });
  });
});
