...
test.describe("Verify editor printbase", () => {
  ...
  test("[Phub] Verify show editor khi add more product @SB_PRH_EDT_PH_4", async ({ dashboard, conf }) => {
  ...
  test("[Phub] Verify show product when delete/remove base product  @SB_PRH_EDT_PH_6", async ({ dashboard }) => {
  ...
  test("[Phub] Verify add thành công layer @SB_PRH_EDT_PH_18", async ({ dashboard, conf, snapshotFixture }) => {
    await printbase.addProductFromCatalog(productInfo.category, productInfo.product_name);
    ...
    test(`${caseData.description} @${caseData.case_id}`, async ({ dashboard, snapshotFixture }) => {
      ...
      await test.step("Verify hiển thị default variant (color, size)", async () => {
        ...
        await snapshotFixture.verify({
          ...
          snapshotName: `${picture.default_variant}`,
          snapshotOptions: {
            maxDiffPixelRatio: maxDiffPixelRatio,
            threshold: threshold,
            maxDiffPixels: maxDiffPixels,
      ...
      await test.step("Select variant for base product", async () => {
        ...
        await snapshotFixture.verify({
          page: dashboard,
          selector: printbase.xpathVariantEditor,
          snapshotName: `${picture.selected_variant}`,
          snapshotOptions: {
            maxDiffPixelRatio: maxDiffPixelRatio,
            threshold: threshold,
            maxDiffPixels: maxDiffPixels,
  ...
  test("[Phub] Verify show ảnh preview khi hover color and select base @SB_PRH_EDT_PH_17", async ({
    dashboard,
    conf,
    snapshotFixture,
  }) => {
    ...
    await test.step("Click button Add text > Click button Preview> Hover color> Verify mockup Preview", async () => {
      ...
      await snapshotFixture.verify({
        ...
        snapshotName: `${picture.preview_with_color}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
    ...
    await test.step("Select base khác >Verify mockup preview", async () => {
      ...
      await snapshotFixture.verify({
        page: dashboard,
        selector: printbase.xpathMockupPreviewOnEditor,
        snapshotName: `${picture.preview_with_base}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
  ...
  test("[Phub] Verify show layer with actions bar @SB_PRH_EDT_PH_21", async ({ dashboard, conf, snapshotFixture }) => {
    test.setTimeout(conf.suiteConf.timeout);
    await printbase.addProductFromCatalog(productInfo.category, productInfo.product_name);
    await printbase.removeLiveChat();
    await printbase.addNewLayer(layerText);
    ...
    await test.step("Click button [Reset all] > Verify show layer", async () => {
      ...
      await snapshotFixture.verify({
        page: dashboard,
        snapshotName: `${productInfo.image_reset_all}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
  ...
  test("[Phub] Verify show layer when delete layer @SB_PRH_EDT_PH_22", async ({ dashboard, conf, snapshotFixture }) => {
    test.setTimeout(conf.suiteConf.timeout);
    const layerAdd = conf.caseConf.layers_add;
    await printbase.addProductFromCatalog(productInfo.category, productInfo.product_name);
    await printbase.removeLiveChat();
    await test.step("Add layer >Add thêm base product > add 1 layer text + 1 layer image> Delete layer", async () => {
      await printbase.addNewLayer(layerText);
      await printbase.addOrRemoveProduct(productInfo.category, productInfo.add_product);
      ...
      await snapshotFixture.verify({
        page: dashboard,
        snapshotName: `${picture.delete_layer}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
    ...
    await test.step("Chọn base product khác  > verify show layer của base", async () => {
      ...
      await snapshotFixture.verify({
        page: dashboard,
        snapshotName: `${picture.base_delete_layer}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
    ...
    await test.step("Delete layer từ action bar > Verify show layer", async () => {
      ...
      await snapshotFixture.verify({
        page: dashboard,
        snapshotName: `${picture.delete_on_action_bar}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
  ...
  test("[Phub] Verify hiển thị mockup sau khi setup mockup @SB_PRH_EDT_PH_23", async ({
    dashboard,
    conf,
    snapshotFixture,
  }) => {
    await test.step("Chọn base products > Click button [Create new campaign] > Verify list mockup default", async () => {
      ...
      await snapshotFixture.verify({
        ...
        snapshotName: `${picture.mockup_default}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
    ...
    await test.step("Click on link text [Update mockup] > đổi vị trí mockup> Untick mockup > Click button Save", async () => {
      ...
      await snapshotFixture.verify({
        page: dashboard,
        selector: printbase.xpathListMockupEditor,
        snapshotName: `${picture.mockup_select}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
  ...
  test("[Phub] Verify hiển thị lỗi khi setup mockup khi bỏ chọn tới mokup reqquired @SB_PRH_EDT_PH_26", async ({
    dashboard,
    conf,
  }) => {
  ...
  test("[Phub] Verify thứ tự các layer khi thực hiện action drag @SB_PRH_EDT_PH_32", async ({
    dashboard,
    conf,
    snapshotFixture,
  }) => {
    ...
    await test.step("Chọn base [AOP Hoodie] > Click button [Create new campaign] > Add layer (layer text, layer image) cho Front side", async () => {
      ...
      await snapshotFixture.verify({
        page: dashboard,
        snapshotName: `${picture.add_layer}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
    ...
    await test.step("Drag Layer text lên vị trí layer Image > Click button Preview > Verify show Layer", async () => {
      ...
      await snapshotFixture.verify({
        page: dashboard,
        snapshotName: `${picture.drag_layer}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
    ...
    await test.step("Drag layer text xuống Back side > Verify show Layer", async () => {
      ...
      await snapshotFixture.verify({
        page: dashboard,
        snapshotName: `${picture.drag_layer_side}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
  ...
  test("[Phub] Verify thông tin Pricing page @SB_PRH_EDT_PH_38", async ({ dashboard, conf, snapshotFixture }) => {
    ...
    await test.step("Chọn [Apparel] tab> Chọn base product > Click button [Create new campaign]> Click button Continue", async () => {
      ...
      await snapshotFixture.verify({
        ...
        snapshotName: `${picture.pricing_default}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
    ...
    await test.step("Click Back lại màn Editor> Add layer text > Select variant> Click button Continue > Click Set individual price", async () => {
      ...
      await snapshotFixture.verify({
        ...
        snapshotName: `${picture.pricing_variant}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
  ...
  test("[Phub] Verify chức năng select product main image @SB_PRH_EDT_PH_42", async ({
    dashboard,
    conf,
    snapshotFixture,
  }) => {
  ...
  test("[Phub] Verify show layer when sync layer @SB_PRH_EDT_PH_43", async ({ dashboard, conf, snapshotFixture }) => {
    const layerAdd = conf.caseConf.layers_add;
    ...
    await test.step("Add thêm base product > check show layer", async () => {
      await printbase.addOrRemoveProduct(productInfo.category_add, productInfo.base_add1);
      await printbase.waitForElementVisibleThenInvisible(printbase.xpathIconLoading);
      await printbase.removeLiveChat();
      await snapshotFixture.verify({
        page: dashboard,
        snapshotName: `${picture.sync_layer_base2D}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
    ...
    await test.step("Delete layer text > chọn button On this product > Chọn base đầu tiên > Verify show layer", async () => {
      ...
      await snapshotFixture.verify({
        page: dashboard,
        selector: printbase.xpathMockupPreviewOnEditor,
        snapshotName: `${picture.layer_sync_after_delete}`,
        snapshotOptions: {
          maxDiffPixelRatio: maxDiffPixelRatio,
          threshold: threshold,
          maxDiffPixels: maxDiffPixels,
  ...
  test("[Phub] Verify chức năng link giữa các product @SB_PRH_EDT_PH_47", async ({ dashboard, snapshotFixture }) => {
  ...
  test("[Phub] Verify chức năng re-link @SB_PRH_EDT_PH_48", async ({ dashboard, snapshotFixture }) => {
    await test.step("Chọn tab Apparel > Chọn base product>Tắt icon link all base > Add layer text> Verify layer on các base còn lại", async () => {
      await printbase.addProductFromCatalog(productInfo.category, productInfo.product_name);
      await printbase.waitUntilElementVisible(printbase.xpathTitleBaseProductOnEditor(productInfo.base_2D_first));
      await printbase.clickLinkProductEditorCampaign();
  ...
  test("[Phub] Verify chức năng auto sync layer @SB_PRH_EDT_PH_46", async ({ dashboard, conf, snapshotFixture }) => {
    const layerEdit = conf.caseConf.layer_edit;
