...
test.describe("Create campaign thành cong", () => {
  const confHomeLiving = loadData(__dirname, "DATA_HOME_LIVING");
  for (let i = 0; i < confHomeLiving.caseConf.data.length; i++) {
    const caseData = confHomeLiving.caseConf.data[i];

    test(`@${caseData.case_id} ${caseData.description}`, async ({ dashboard, snapshotFixture, context }) => {
      test.setTimeout(confHomeLiving.suiteConf.timeout);
      const printbasePage = new PrintBasePage(dashboard, confHomeLiving.suiteConf.domain);
      ...
      await test.step(
        ...
        async () => {
          ...
          await printbasePage.deleteAllCampaign(confHomeLiving.suiteConf.password);
      ...
      await test.step("Verify campaign ngoài SF", async () => {
        ...
        const campaignSF = new SFProduct(SFPage, confHomeLiving.suiteConf.domain);
  ...
  const confDuplicateHomeLiving = loadData(__dirname, "DATA_DUPLICATE_HOME_LIVING");
  for (let i = 0; i < confDuplicateHomeLiving.caseConf.data.length; i++) {
    const caseData = confDuplicateHomeLiving.caseConf.data[i];

    test(`@${caseData.case_id} ${caseData.description}`, async ({ dashboard, snapshotFixture, context }) => {
      test.setTimeout(confDuplicateHomeLiving.suiteConf.timeout);
      const printbasePage = new PrintBasePage(dashboard, confDuplicateHomeLiving.suiteConf.domain);
      const picture = caseData.picture;
      ...
      await test.step(
        ...
        async () => {
          ...
          await printbasePage.deleteAllCampaign(confDuplicateHomeLiving.suiteConf.password);
          await printbasePage.navigateToMenu("Campaigns");
      ...
      await test.step("Verify campaign ngoài SF", async () => {
        const [SFPage] = await Promise.all([context.waitForEvent("page"), await printbasePage.openCampaignSF()]);
        const campaignSF = new SFProduct(SFPage, confDuplicateHomeLiving.suiteConf.domain);
        await campaignSF.waitResponseWithUrl("/assets/landing.css", 60000);
        await campaignSF.waitUntilElementVisible(campaignSF.xpathProductMockupSlide);
        await campaignSF.page.waitForTimeout(5000);
        await campaignSF.waitForImagesMockupLoaded();
