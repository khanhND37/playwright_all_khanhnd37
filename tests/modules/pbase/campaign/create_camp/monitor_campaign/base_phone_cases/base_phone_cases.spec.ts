...
test.describe("Create campaign thành cong", () => {
  const confPCase = loadData(__dirname, "DATA_PHONE_CASE");
  for (let i = 0; i < confPCase.caseConf.data.length; i++) {
    const caseData = confPCase.caseConf.data[i];

    test(`@${caseData.case_id} ${caseData.description}`, async ({ dashboard, snapshotFixture, context }) => {
      test.setTimeout(confPCase.suiteConf.timeout);
      const printbasePage = new PrintBasePage(dashboard, confPCase.suiteConf.domain);
      ...
      await test.step(
        ...
        async () => {
          ...
          await printbasePage.deleteAllCampaign(confPCase.suiteConf.password);
      ...
      await test.step("Verify campaign ngoài SF", async () => {
        ...
        const campaignSF = new SFProduct(SFPage, confPCase.suiteConf.domain);
  ...
  const confDuplicatePhoneCase = loadData(__dirname, "DATA_DUPLICATE_PHONE_CASE");

  for (let i = 0; i < confDuplicatePhoneCase.caseConf.data.length; i++) {
    const caseData = confDuplicatePhoneCase.caseConf.data[i];

    test(`@${caseData.case_id} ${caseData.description}`, async ({ dashboard, snapshotFixture, context }) => {
      test.setTimeout(confDuplicatePhoneCase.suiteConf.timeout);
      const printbasePage = new PrintBasePage(dashboard, confDuplicatePhoneCase.suiteConf.domain);
      const picture = caseData.picture;
      ...
      await test.step(
        ...
        async () => {
          ...
          await printbasePage.deleteAllCampaign(confDuplicatePhoneCase.suiteConf.password);
          await printbasePage.navigateToMenu("Campaigns");
      ...
      await test.step("Verify campaign ngoài SF", async () => {
        const [SFPage] = await Promise.all([context.waitForEvent("page"), await printbasePage.openCampaignSF()]);
        const campaignSF = new SFProduct(SFPage, confDuplicatePhoneCase.suiteConf.domain);
        await campaignSF.waitResponseWithUrl("/assets/landing.css", 60000);
        await campaignSF.waitUntilElementVisible(campaignSF.xpathProductMockupSlide);
        await campaignSF.page.waitForTimeout(5000);
        await campaignSF.waitForImagesMockupLoaded();
  ...
  const confDrinkware = loadData(__dirname, "DATA_DRINKWARE");
  for (let i = 0; i < confDrinkware.caseConf.data.length; i++) {
    const caseData = confDrinkware.caseConf.data[i];

    test(`@${caseData.case_id} ${caseData.description}`, async ({ dashboard, snapshotFixture, context }) => {
      test.setTimeout(confDrinkware.suiteConf.timeout);
      const printbasePage = new PrintBasePage(dashboard, confDrinkware.suiteConf.domain);
      const campaignsInfos = caseData.campaign_info;
      const picture = caseData.picture;
      await test.step(
        '1. Vào "Catalog" ' +
        ...
        async () => {
          await printbasePage.navigateToMenu("Campaigns");
          await printbasePage.searchWithKeyword(campaignsInfos.pricing_info.title);
          await printbasePage.deleteAllCampaign(confDrinkware.suiteConf.password);
          await printbasePage.navigateToMenu("Catalog");
          const campainId = await printbasePage.launchCamp(campaignsInfos);
          const isAvailable = await printbasePage.checkCampaignStatus(campainId, ["available"], 30 * 60 * 1000);
          expect(isAvailable).toBeTruthy();
      ...
      await test.step("Verify campaign ngoài SF", async () => {
        const [SFPage] = await Promise.all([context.waitForEvent("page"), await printbasePage.openCampaignSF()]);
        const campaignSF = new SFProduct(SFPage, confDrinkware.suiteConf.domain);
        await campaignSF.waitResponseWithUrl("/assets/landing.css", 60000);
        await campaignSF.waitUntilElementVisible(campaignSF.xpathProductMockupSlide);
        await campaignSF.page.waitForTimeout(5000);
        await campaignSF.waitForImagesMockupLoaded();
  ...
  const confDuplicateDrinkware = loadData(__dirname, "DATA_DUPLICATE_DRINKWARE");

  for (let i = 0; i < confDuplicateDrinkware.caseConf.data.length; i++) {
    const caseData = confDuplicateDrinkware.caseConf.data[i];

    test(`@${caseData.case_id} ${caseData.description}`, async ({ dashboard, snapshotFixture, context }) => {
      test.setTimeout(confDuplicateDrinkware.suiteConf.timeout);
      const printbasePage = new PrintBasePage(dashboard, confDuplicateDrinkware.suiteConf.domain);
      const picture = caseData.picture;
      const campaignOrigin = caseData.campaign_origin;
      const campaignDuplicate = caseData.campaign_duplicate;
      await test.step(
        ...
        async () => {
          await printbasePage.navigateToMenu("Campaigns");
          await printbasePage.searchWithKeyword(campaignDuplicate.title);
          await printbasePage.deleteAllCampaign(confDuplicateDrinkware.suiteConf.password);
          await printbasePage.navigateToMenu("Campaigns");
          await printbasePage.searchWithKeyword(campaignOrigin);
          await printbasePage.duplicateCampaign(campaignOrigin, true);
          await printbasePage.clickOnBtnWithLabel("Continue");
      ...
      await test.step("Verify campaign ngoài SF", async () => {
        const [SFPage] = await Promise.all([context.waitForEvent("page"), await printbasePage.openCampaignSF()]);
        const campaignSF = new SFProduct(SFPage, confDuplicateDrinkware.suiteConf.domain);
        await campaignSF.waitResponseWithUrl("/assets/landing.css", 60000);
        await campaignSF.waitUntilElementVisible(campaignSF.xpathProductMockupSlide);
        await campaignSF.page.waitForTimeout(5000);
        await campaignSF.waitForImagesMockupLoaded();
