...
test.describe("Create campaign thành cong", () => {
  const confje = loadData(__dirname, "DATA_JEWELRY");
    ...
    test(`@${caseData.case_id} ${caseData.description}`, async ({ dashboard, snapshotFixture, context }) => {
      ...
      await test.step(
        ...
        async () => {
          ...
          await printbasePage.deleteAllCampaign(confje.suiteConf.password);
      ...
      await test.step("Verify campaign ngoài SF", async () => {
        ...
        const campaignSF = new SFProduct(SFPage, confje.suiteConf.domain);
  ...
  const confDuplicateJe = loadData(__dirname, "DATA_DUPLICATE_JEWELRY");
  for (let i = 0; i < confDuplicateJe.caseConf.data.length; i++) {
    const caseData = confDuplicateJe.caseConf.data[i];

    test(`@${caseData.case_id} ${caseData.description}`, async ({ dashboard, snapshotFixture, context }) => {
      test.setTimeout(confDuplicateJe.suiteConf.timeout);
      const printbasePage = new PrintBasePage(dashboard, confDuplicateJe.suiteConf.domain);
      const picture = caseData.picture;
      ...
      await test.step(
        ...
        async () => {
          ...
          await printbasePage.deleteAllCampaign(confDuplicateJe.suiteConf.password);
          await printbasePage.navigateToMenu("Campaigns");
      ...
      await test.step("Verify campaign ngoài SF", async () => {
        const [SFPage] = await Promise.all([context.waitForEvent("page"), await printbasePage.openCampaignSF()]);
        const campaignSF = new SFProduct(SFPage, confDuplicateJe.suiteConf.domain);
        await campaignSF.waitResponseWithUrl("/assets/landing.css", 60000);
        await campaignSF.waitUntilElementVisible(campaignSF.xpathProductMockupSlide);
        await campaignSF.page.waitForTimeout(5000);
        await campaignSF.waitForImagesMockupLoaded();
