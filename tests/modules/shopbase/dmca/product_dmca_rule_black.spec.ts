...
test.describe("Verify dmca sbase", () => {
  ...
  const confData = loadData(__dirname, "DATA_DRIVEN_BLACK");
  confData.caseConf.data.forEach(baseData => {
    test(`@${baseData.case_id} ${baseData.description}`, async ({ conf, scheduler }) => {
      ...
      await test.step(`1. Click vào tab Products > Click vào btn Add product> Tạo 2 products chứa các title lần lượt là scamdmcasbaseblack 01, scamdmcasbaseblack 02 > Click vào btn Save`, async () => {
      ...
      await test.step(`2. Vào app Print Hub tạo mới campaign > Click vào btn Launch`, async () => {
        // create campaign
      ...
      await test.step(`5. Sau 30ph merchant chưa xóa các products vi phạm`, async () => {
