...
test.describe("plbase-order tracking page", async () => {
  ...
  test("[SB] Verify UI trang Thankyou/ Order status @TC_PLB_OTP_2", async ({ conf, context, dashboard }) => {
    ...
    await test.step("Check message trả về", async () => {
      const trackingStep = conf.caseConf.tracking_steps_Sb;
