...
test.describe("Checkout express", () => {
  ...
  test(`@SB_CHE_NEW_EF_53 [Pbase]-[Desktop]-[Theme Inside] Check out với Paypal express và kiểm tra order detail trên dashboard và trên cổng, order có PPC, setting 1 page checkout`, async ({
    conf,
    page,
    authRequest,
  ...
  test(`@SB_CHE_NEW_EF_65 [Pbase]-[Desktop]-[Theme Inside] Kiểm tra order có PPC thỏa mãn free shipping có cap value khi checkout qua luồng express payment`, async ({
    conf,
    page,
    authRequest,
    dashboard,
    ...
    await test.step(`- Buyer nhấn confirm order trên popup`, async () => {
      ...
      orderSummaryInfo = await checkoutPage.getOrderInfoAfterCheckout();
      expect(isEqual(orderSummaryInfo.totalSF, cartSummary.total_price + cartPPCSummary.total_price, 0.01)).toBe(true);
      expect(Number(orderSummaryInfo.shippingSF)).toBe(cartSummary.shipping_value + cartPPCSummary.shipping_value);
    });
    ...
    await test.step(`- Login vào Dashboard- Vào Order detail của order vừa tạo`, async () => {
      ...
      await orderPage.goToOrderByOrderId(orderSummaryInfo.orderId, "pbase");
      const orderDetailSummary = await orderPage.getOrderSummaryShopBaseInOrderDetail();
      const shipInfoInOrdDetail = await orderPage.getShippingValuesInOrdDetail("Standard shipping");

      expect(isEqual(orderDetailSummary.total, orderSummaryInfo.totalSF, 0.01)).toBe(true);
      expect(Number(shipInfoInOrdDetail.newShippingValue)).toBe(
        cartSummary.shipping_value + cartPPCSummary.shipping_value,
      );
      expect(
        isEqual(
          shipInfoInOrdDetail.originShipFee - Number(shipInfoInOrdDetail.newShippingValue),
          conf.caseConf.max_shipping_value,
          0.01,
        ),
      ).toBe(true);
    });
  ...
  test(`@SB_CHE_NEW_EF_Pr_771 [Printbase][Theme ver 2][Product page] Checkout thành công qua Buy with Paypal có add PPC`, async ({
    conf,
    page,
    authRequest,
    authRequestWithExchangeToken,
