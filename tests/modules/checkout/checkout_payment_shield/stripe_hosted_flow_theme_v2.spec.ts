...
test.describe("Checkout với Stripe hosted thành công", () => {
  const caseName = "TC_STRIPE_HOSTED";
...
test.describe("Checkout với Stripe hosted không thành công", () => {
  test(`@SB_SB_SF_HD_PS_49 [SF theme v2] Checkout fail khi sử dụng proxy domain với cổng stripe EU`, async ({
    page,
    conf,
    authRequest,
  }) => {
    ...
    await test.step(`Tại SF:- Add product to cart > Checkout- Fill info`, async () => {
      ...
      await homePage.gotoHomePage();
      await homePage.selectStorefrontCurrencyV2(shippingAddress.country_name);
      await checkoutAPI.addProductThenSelectShippingMethod(productsCheckout, buyerEmail, shippingAddress);
      checkout = await checkoutAPI.openCheckoutPageByToken();
      const checkoutURL = checkout.page.url();
      expect(checkoutURL).toContain(domain);
    });
