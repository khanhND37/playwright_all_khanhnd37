...
test.describe(`[Sbase] Checkout khi ẩn cardholder name one page`, () => {
  test.beforeEach(async ({ dashboard, conf, theme }) => {
    ...
    await paymentSetPage.settingHideCardholderName(conf.suiteConf.payment_method, true);
    const settingTheme = new SettingThemeAPI(theme);
    await settingTheme.editCheckoutLayout("one-page");
  });
  test(`[Shopbase] Buyer checkout 1 page qua spay khi Merchant setting hide cardhoder name @SB_SC_578`, async ({
    page,
    conf,
    request,
    token,
    ...
    const connectedAcc = conf.suiteConf.connected_account;
    ...
    await test.step(`- Buyer checkout đến bước payment method:  + Payment method: Stripe`, async () => {
      ...
      await expect(checkout.spayCardNameLoc).toBeHidden();
    });
...
test.describe(`[Sbase] Checkout khi hiện cardholder name với 3-step checkout`, () => {
  test.beforeEach(async ({ dashboard, conf, theme }) => {
    ...
    const paymentSetPage = new PaymentProviderPage(dashboard, domain);
    await paymentSetPage.goto(`admin/settings/payments`);
    await paymentSetPage.settingHideCardholderName(conf.suiteConf.payment_method, false);
    const settingTheme = new SettingThemeAPI(theme);
    await settingTheme.editCheckoutLayout("one-page");
  });
  test(`[Shopbase] Buyer checkout 3 page qua spay khi Merchant setting hiển thị cardhoder name @SB_SC_577`, async ({
    page,
    conf,
    request,
    token,
    ...
    await test.step(`- Buyer checkout đến bước payment method:  + Payment method: Stripe`, async () => {
      ...
      checkout = new SFCheckout(page, domain);
      await checkout.selectPaymentMethod(paymentMethod);
      await expect(checkout.spayCardNameLoc).toBeVisible();
    });
