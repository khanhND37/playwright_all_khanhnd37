...
import { SbWebBuilderBcreB } from "./radius-ui-setting";

test.describe("Verify setting radius của customization service block", () => {
  let dashboard: Page;
  ...
  test(`@SB_WEB_BUILDER_BCRE_BCS_13 Verify block customization service khi thực hiện setting radius`, async ({
    page,
    conf,
    context,
    snapshotFixture,
      ...
      await test.step(step.name, async () => {
        ...
        for (const align of aligns) {
          ...
          await snapshotFixture.verify({
            page: wbCsPage.page,
            selector: wbCsPage.getXpathSidebarSetting("border_radius"),
...
test.describe("Verify setting radius của question block", () => {
  let dashboard: Page;
  let wbCsPage: WbPageCustomizationService;
  let productHandle: string;
  ...
  test(`@SB_WEB_BUILDER_BCRE_BSQ_14 Verify block question khi thực hiện setting radius`, async ({
    page,
    conf,
    context,
    snapshotFixture,
