...
import { SbWebBuilderBcreB } from "./position-ui-setting";

test.describe("Verify setting position của customization service block", () => {
  let dashboard: Page;
  ...
  test(`@SB_WEB_BUILDER_BCRE_BCS_08 Verify block customization service khi thực hiện setting shadow`, async ({
    page,
    conf,
    context,
    snapshotFixture,
...
test.describe("Verify setting position của question block", () => {
  let dashboard: Page;
  let wbCsPage: WbPageCustomizationService;
  let productHandle: string;
  ...
  test(`@SB_WEB_BUILDER_BCRE_BSQ_09 Verify block question khi thực hiện setting shadow`, async ({
    page,
    conf,
    context,
    snapshotFixture,
