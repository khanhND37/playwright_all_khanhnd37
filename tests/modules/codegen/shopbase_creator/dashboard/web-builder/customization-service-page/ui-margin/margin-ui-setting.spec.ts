...
import { SbWebBuilderBcreB } from "./margin-ui-setting";

test.describe("Verify setting margin của customization service block", () => {
  let dashboard: Page;
  ...
  test(`@SB_WEB_BUILDER_BCRE_BCS_15 Verify block customization service khi thực hiện setting margin`, async ({
    page,
    conf,
    context,
    snapshotFixture,
      ...
      await test.step(step.name, async () => {
        ...
        for (const align of aligns) {
          ...
          await snapshotFixture.verify({
            page: wbCsPage.page,
            selector: wbCsPage.getXpathSidebarSetting("margin"),
...
test.describe("Verify setting margin của question block", () => {
  let dashboard: Page;
  let wbCsPage: WbPageCustomizationService;
  let productHandle: string;
  ...
  test(`@SB_WEB_BUILDER_BCRE_BSQ_16 Verify block question khi thực hiện setting margin`, async ({
    page,
    conf,
    context,
    snapshotFixture,
