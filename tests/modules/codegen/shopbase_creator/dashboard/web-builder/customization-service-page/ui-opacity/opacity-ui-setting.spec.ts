...
import { SbWebBuilderBcreBcs12 } from "./opacity-ui-setting";

test.describe("Verify setting opacity của customization service block", () => {
  let dashboard: Page;
  ...
  test(`@SB_WEB_BUILDER_BCRE_BCS_12 Verify block customization service khi thực hiện setting opacity`, async ({
    page,
    conf,
    context,
    snapshotFixture,
    ...
    const caseConf = conf.caseConf as SbWebBuilderBcreBcs12;
      ...
      await test.step(step.name, async () => {
        ...
        for (const align of aligns) {
          ...
          await snapshotFixture.verify({
            page: wbCsPage.page,
            selector: wbCsPage.getXpathSidebarSetting("opacity"),
...
test.describe("Verify setting opacity của question block", () => {
  let dashboard: Page;
  let wbCsPage: WbPageCustomizationService;
  let productHandle: string;
  ...
  test(`@SB_WEB_BUILDER_BCRE_BSQ_13 Verify block question khi thực hiện setting opacity`, async ({
    page,
    conf,
    context,
    snapshotFixture,
