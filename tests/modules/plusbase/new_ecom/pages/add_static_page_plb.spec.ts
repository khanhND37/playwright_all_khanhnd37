...
test.describe("Add static page on shop template plusbase", () => {
  ...
  test.beforeAll(async ({ conf, browser }) => {
    ...
    const shopTemplateConf = Object.assign({ api: conf.suiteConf.api }, conf.suiteConf.plb_template);
    ...
    templatePage = new OtherPage(tmplPage.page, conf.suiteConf.plb_template.domain);
  ...
  test("@SB_NEWECOM_SP_30 - Verify add page trong shop template", async ({ conf, builder }) => {
