...
import { StorefrontInfo } from "./filter_order_plb_multiple_storefronts";
...
test.describe("Verify filter order with multiple storefront", () => {
  ...
  test("@SB_ORD_MSF_11 Checkout 2 SF và check filter tại trang list all order và thông tin order detail khi store config multiple SF", async ({
    request,
    page,
  }) => {
    // checkout main SF
    ...
    await test.step(`Tại Dashboard > Order: Filter order theo storefront domain 1`, async () => {
      await ordersPage.goto(`https://${domain1}/admin/plb/orders`);
      await ordersPage.clickButtonMoreFilters("PlusBase");
    ...
    await test.step(`Vào Order detail của order vừa tạo ở SF1 để kiểm tra storefront domain của order trong order detail`, async () => {
      ...
      await ordersPage.page.waitForSelector(`text='${orderName1}'`);
    ...
    await test.step(`Vào Order detail của order vừa tạo ở SF2 để kiểm tra storefront domain của order trong order detail`, async () => {
      ...
      await ordersPage.page.waitForSelector(`text='${orderName2}'`);
      expect(await ordersPage.isDisplayStorefrontDomain(storefront2Info)).toBe(true);
    });
  });

  test(`@SB_ORD_MSF_22 Check export file sau khi filter theo SF domain tại trang list all order`, async ({ conf }) => {
    await test.step(`Tại Dashboard > Order, filter order theo storefront domain 1`, async () => {
      await ordersPage.goto(`https://${domain1}/admin/plb/orders`);
      await ordersPage.clickButtonMoreFilters("PlusBase");
    ...
    await test.step(`Tại màn list order, select order > Click "Export order". Select export order theo Selected number orders
      ...
      filePath = await ordersPage.downloadFileExportOrder("plusbase");
