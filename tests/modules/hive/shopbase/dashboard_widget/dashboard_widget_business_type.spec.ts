...
import { SbHmWg4 } from "./dashboard_widget_business_type";

test.describe("Dashboard widget business type", async () => {
  ...
  test(`Check điều kiện hiển thị widget theo business type @SB_HM_WG_4`, async ({
    page,
    cConf,
    token,
    context,
    ...
    const caseConf = cConf as SbHmWg4;

    await test.step(`Tạo thành công widget config business type khác nhau`, async () => {
    ...
    await test.step(`Login dashboard shop > kiểm tra hiển thị widget ở home page`, async () => {
      for (const shopData of caseConf.shops) {
        ...
        let found = false;
        for (const widgetData of caseConf.widgets) {
          for (const businessTypeWidget of widgetData.tabs.logic.business_types) {
            if (businessTypeWidget === shopData.business_types) {
              found = true;
              break;
            }
          }
        ...
        // verify không hiển thị widget ở shop không thỏa mãn điều kiện business type
        else {
          await homePage.loginWithToken(accessToken);
          await homePage.waitUtilNotificationIconAppear();
          for (const widgetData of caseConf.widgets) {
