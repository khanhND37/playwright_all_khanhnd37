...
test.describe("Activate plan without card in free trial", () => {
  ...
  test(`[Pbase] Kiểm tra active plan cho store đang in free trial và chọn method top-up là Credit card @SB_ACT_PL_WC_8`, async ({
    conf,
  }) => {
...
test.describe(`Charge failed`, () => {
  ...
  test(`[Pbase] Kiểm tra store hết free trial và bị re-charge subscription fee không thành công
    khi available balance của store không đủ @SB_ACT_PL_WC_11`, async ({}) => {
  ...
  test(`[Pbase] Kiểm tra store hết free trial và bị re-charge  subscription fee thành công khi
    available balance của store không đủ @SB_ACT_PL_WC_12`, async ({ conf, page }) => {
...
test.describe("Activate plan without card after free trial", () => {
  ...
  test(`[Pbase] Kiểm tra active plan after free trial đối với country được support active without card,
    available balance của account lớn hơn subscription fee @SB_ACT_PL_WC_17`, async ({ conf }) => {
    await test.step(`Mở store dashboard > Verify hiển thị page Confirm plan và các giá trị tương ứng`, async () => {
      const hasActivatePlanButton = await confirmPlanPage.activatePbPlanButtonLoc.isVisible();
      if (!hasActivatePlanButton) {
        await accountPage.selectShopByName(conf.caseConf.domain);
      }

      await expect(confirmPlanPage.activatePbPlanButtonLoc).toBeVisible();

      await confirmPlanPage.activatePbPlanButtonLoc.click();
  ...
  test(`[Pbase] Kiểm tra active plan after free trial đối với country được support active without card,
    available balance của account nhỏ hơn subscription fee @SB_ACT_PL_WC_18`, async ({ conf, page, hiveSBase }) => {
    await test.step(`Pre conditions`, async () => {
      //charge available to zero
...
test.describe("Logback store", () => {
  ...
  test(` [Pbase] Kiểm tra log back to store với store đã end free trial và user region được
    support activate without card (Available sufficient) @SB_ACT_PL_WC_29`, async ({ page, conf, api }) => {
  ...
  test(` [Pbase] Kiểm tra log back to store với store đã end free trial và user region
    được support activate without card (Available insufficient) @SB_ACT_PL_WC_30`, async ({
    page,
    conf,
    api,
    hiveSBase,
