...
test.describe("Conversion Analytics", () => {
  ...
  test(`@SB_SB_DP_IMP_CRP_UCA_17 [Roller][Shopbase] Kiểm tra logic call action và analytics ghi nhận đủ các step khi thực hiện 1 session bình thường`, async ({
    page,
    conf,
  }) => {
    ...
    await test.step(`
      ...
      await Promise.all([checkout.selectPaymentMethod(), checkout.waitForEventCompleted(domain, "fill_payment_info")]);
    });
