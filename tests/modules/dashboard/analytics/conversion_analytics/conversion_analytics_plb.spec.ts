...
test.describe("Verify conversion analytics on PLB", () => {
  ...
  test(`@SB_ANA_CONVER_09 On PLB, verify Conversion Analytics theo Hour cùng các điều kiện khác`, async ({
    conf,
    authRequest,
  }) => {
    ...
    await test.step(`Ở Dashboard, Analytics > Conversion analytics, filter: Group by Hour, Select countries (có data), Last month`, async () => {
      await conversionPage.clearFilterCountries();
    ...
    await test.step(`Ở Dashboard, Analytics > Conversion analytics, click button Export`, async () => {
      await conversionPage.page.click(conversionPage.xpathExport);
  ...
  test(`@SB_ANA_CONVER_10 On PLB, verify Conversion Analytics theo Day cùng các điều kiện khác`, async ({
    conf,
    authRequest,
  }) => {
  ...
  test(`@SB_ANA_CONVER_11 On PLB, verify Conversion Analytics theo Week cùng các điều kiện khác`, async ({
    conf,
    authRequest,
  }) => {
    ...
    await test.step(`Ở Dashboard, Analytics > Conversion analytics, click button Export`, async () => {
      ...
      // TACH PHAN CHECK MAIL SANG CASE KHAC
      // await conversionPage.page.click('(//span[child::b[text()="All"]])[1]');
      // mailinator = await getMailinatorInstanceWithProxy(page);
      // await mailinator.accessMail(conf.suiteConf.staff_name);
      // while (!(await mailinator.checkMailVisible("Export Report Data"))) {
      //   await mailinator.page.reload();
      ...
      // const feedFileData = await readFileCSV(feedFile, "\t");
      // expect(feedFileData[0][0]).toContain(
      //   (await (await conversionPage.genLocatorValueWithColumn("View all (VA)", "body", 2)).textContent()).trim(),
      // );
      // expect(feedFileData[0][0]).toContain(
      // (await (await conversionPage.genLocatorValueWithColumn("View product (VP)", "body", 2)).textContent()).trim(),
      // );
      // expect(feedFileData[0][0]).toContain(
      //   (await (await conversionPage.genLocatorValueWithColumn("Add to cart (AC)", "body", 2)).textContent()).trim(),
      // );
      // expect(feedFileData[0][0]).toContain(
      //   (await (await conversionPage.genLocatorValueWithColumn("Purchase (PU)", "body", 2)).textContent()).trim(),
      // );
    });
  });

  test(`@SB_ANA_CONVER_12 On PLB, verify Conversion Analytics theo Month cùng các điều kiện khác`, async ({
    conf,
    authRequest,
  }) => {
