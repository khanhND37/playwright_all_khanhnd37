...
test.describe("Feed file shipping_kiểm tra sync shipping product lên GMC bằng cách upload file csv Feed file cho shop PlusBase", async () => {
  const data = "FEED_FILE_SHIPPING_PLUSBASE";
  const conf = loadData(__dirname, data);
      ...
      test(`@${caseId} ${feedInfo.feed_title}`, async ({ dashboard, page, authRequest, ggPage, scheduler }) => {
        ...
        await test.step(
          ...
          async () => {
            ...
            if (await ggPage.locator(gmcPage.xpathWarningFeedAlready).isHidden()) {
              ...
              expect(await ggPage.locator(gmcPage.xpathHeaderFeedPage).isVisible()).toBeTruthy();
            }
          },
        );
...
test.describe("Feed file shipping_kiểm tra feed file status = Processing khi tạo feed file, chọn country không có product nào thỏa mãn cho shop Print/Plus base", async () => {
  test("@SB_MAR_SALES_SC_FEED_GMC_SYNC_SHIP_FEE_22", async ({ dashboard, authRequest, conf, scheduler }) => {
    feedInfo = conf.caseConf.feed_info;
    productPage = new ProductPage(dashboard, conf.suiteConf.domain);

    await test.step("- Tại Product feed -> Click button Add product feed- click chọn Other Feed With Shipping- input Feed name, select các options -> click Save", async () => {
      await productPage.goToProductFeedList();
      isFeedDisapperDashboard = await dashboard
        .locator(productPage.getXpathProductFeed(feedInfo.feed_title))
        .isVisible();
      if (!isFeedDisapperDashboard) {
        await productPage.addProductFeed(feedInfo);
        await expect(dashboard.locator(productPage.xpathLabelProductFeedURL)).toBeVisible();
    ...
    await test.step("kiểm tra hiển thị tại field Product feed URL", async () => {
      await productPage.goToProductFeedList();
      await productPage.goToProductFeedDetail(feedInfo.feed_title);
      expect(await dashboard.locator(productPage.xpathStatusProductFeedURLDetail).textContent()).toContain(
        "Processing",
      );

      //delete feed file at store's dashboard and after completed verify
      await productPage.goToProductFeedList();
      if ((await dashboard.locator(productPage.getXpathProductFeed(feedInfo.feed_title)).count()) > 0) {
        await productPage.deleteFeedByAPI(authRequest, feedInfo.feed_title, conf.suiteConf.domain);
      }
