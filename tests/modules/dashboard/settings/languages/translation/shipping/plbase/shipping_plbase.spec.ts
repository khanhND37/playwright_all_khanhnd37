...
test.describe("Automate testcase for entity Shipping(feature translation)", () => {
  ...
  test(`@SB_SET_LG_TLL_61 - [DB+SF - Function] Kiểm tra translate Shipping method - PlusBase`, async ({
    dashboard,
    conf,
  }) => {
    test.slow();
    ...
    await test.step("Setup shipping zone data", async () => {
      // PlusBase only show 1 in 3 shipping Premium Shipping, Standard Shipping, Eco Shipping method  not need edit
      return true;
    });
    /* --- ---*/
