...
test.describe("Verify API bootstrap translation ", () => {
  test(`@SB_SET_TL_DEMO_bootstrap Demmo get translation data from bootstrap`, async ({ authRequest, conf }) => {
...
test.describe("Verify Enable auto translate All pages content - Shopbase ", () => {
  ...
  const caseDataEnable = loadData(__dirname, "DATA_DRIVEN_ENABLE_AUTO_TRANSLATE_SBASE");
...
test.describe("Verify Disable auto translate All pages content - Shopbase ", () => {
  let dashboardPage: TranslationDetail;
  let duplicatedTheme: ShopTheme;
  let dataDBTargetLanguage;
  let data;
  ...
  const caseDataDisable = loadData(__dirname, "DATA_DRIVEN_DISABLE_AUTO_TRANSLATE_SBASE");
