...
test.describe("SB_CHE_CHEN_NE_1PAGE_EU_PAYMENTS Checkout new ecom without setting PPC via EU payment", () => {
  ...
  const casesID = "SB_CHE_CHEN_NE_1PAGE_EU_PAYMENTS";
    ...
    ({
      ...
      test(`@${caseId} ${caseName}`, async ({ page, authRequest, dashboard }) => {
        ...
        await test.step(`Merchant kiểm tra order details trong dashboard`, async () => {
          ...
          const tipAmount = parseFloat(removeCurrencySymbol(await orderPage.getTip()));
          expect(isEqual(actTotalOrder, orderSummaryInfo.totalSF, 0.01)).toBe(true);
          expect(isEqual(tipAmount, orderSummaryInfo.tippingValue, 0.01)).toBe(true);
        });
