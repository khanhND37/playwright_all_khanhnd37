...
test.describe("Checkout new ecom with Spay", () => {
  ...
  const casesID = "SB_CHE_SPAY_NE_3STEPS";
    ...
    ({
      ...
      test(`@${caseId} ${caseName}`, async ({ page, authRequest, dashboard }) => {
        ...
        await test.step(`Merchant kiểm tra order details trong dashboard`, async () => {
          ...
          await orderPage.reloadUntilOrdCapture(undefined, 3);
          // verify order amount
  ...
  test(`@SB_NEWECOM_CO_10 Kiểm tra order detail khi checkout thành công với cổng Spay, checkout 3 steps`, async ({
    page,
    conf,
    authRequest,
    dashboard,
    ...
    //   await mailBox.openOrderConfirmationNotification(orderSummaryInfo.orderName, 4);
    //   // verify total order
    //   const actualTotalOrder = parseFloat(removeCurrencySymbol(await mailBox.getTotalOrder()));
    //   expect(isEqual(actualTotalOrder, orderSummaryInfo.totalSF, 0.01)).toBe(true);
    // });
